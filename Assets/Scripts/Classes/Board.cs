...
namespace Classes
{
    public class Board
    {
        ...
        public int LastHorizontalGridPos { get; private set; }
        ...
        public void PopulateBoard(int[] shipsConfiguration, int verticalSize, int horizontalSize)
        {
            ...
            // Loop generating x ships in random locations which cannot overlap or be next to each other
            for (var i = 0; i < shipsTotalToPlace;)
                while (true)
                {
        ...
        private bool CheckIfTheresSpaceForShip(int vertical, int horizontal)
        {
        ...
        {
            for (var i = 0; i < LastHorizontalGridPos + 1; i++)
            for (var j = 0; j < LastVerticalGridPos + 1; j++)
